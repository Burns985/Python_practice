...
def save_pool():
    # Create the 'Current_Model_Pool' directory if it doesn't exist
...
def load_or_create_model(model_file):
    if os.path.exists(model_file):
        ...
        # If the model file doesn't exist, create a new model
        model = Sequential()
        model.add(Dense(units=7, input_dim=3))
        model.add(Activation("sigmoid"))
        model.add(Dense(units=1))
        ...
        # Optionally, you can save the newly created model
...
if load_saved_pool:
    for i in range(total_models):
        # Load or create the model for a specific index
        model_file = "Current_Model_Pool/model_new" + str(i) + ".keras"
        current_pool[i] = load_or_create_model(model_file)

    # Call the save_pool function here, after loading the models
    save_pool()
...
def main():
    ...
    SOUNDS['die'] = pygame.mixer.Sound('assets/audio/assets_audio_die.wav' + soundExt)
    SOUNDS['hit'] = pygame.mixer.Sound('assets/audio/assets_audio_hit.wav' + soundExt)
    SOUNDS['point'] = pygame.mixer.Sound('assets/audio/assets_audio_point.wav' + soundExt)
    SOUNDS['swoosh'] = pygame.mixer.Sound('assets/audio/assets_audio_swoosh.wav' + soundExt)
    SOUNDS['wing'] = pygame.mixer.Sound('assets/audio/assets_audio_wing.wav' + soundExt)
